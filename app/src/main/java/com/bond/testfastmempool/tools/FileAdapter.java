...
public class FileAdapter {
  ...
  public static void save_byte_buf_local(String file, byte[] data, Context context)  {
    StringBuilder sb  = new StringBuilder(256);
    sb.append(context.getFilesDir().getPath())
        .append("/specnet/").append(file);
    saveFile(sb.toString(),  data);
  }
...
//          }
  ...
  public static ByteBuffer loadAssetByteBuffer(String pathRelative, Context context)  {
    ByteArrayOutputStream fbuf = new ByteArrayOutputStream(2048);
    try {
      //InputStream in = getAssets().open("sub/sample.txt");
      InputStream in = context.getAssets().open(pathRelative);
      byte[] buf = new byte[2048];
    ...
    ByteBuffer byteData = ByteBuffer.wrap(fbuf.toByteArray());
    return byteData;//.order(ByteOrder.nativeOrder());
    //return fbuf.toByteArray();
  }

  private File copyFromAssets(String pathRelative, Context context,
                              String target) throws Exception {
    InputStream in = context.getAssets().open(pathRelative);
    File re = new File(target);
    return re;
  } //copyFromAssets

  public static MappedByteBuffer loadAssetMappedByteBuffer(String pathRelative, Context context)
      throws  Exception  {
    StringBuilder sb  = new StringBuilder(256);
    sb.append(context.getFilesDir().getPath())
        .append("/speccache");
    String dir = sb.toString();
    sb.append("/").append(pathRelative);
    ...
    return  new FileInputStream(filePath).getChannel().map(
        FileChannel.MapMode.READ_ONLY, 0, length);

//    AssetFileDescriptor fileDescriptor = context.getAssets().openFd(pathRelative);
//    FileInputStream inputStream = new FileInputStream(fileDescriptor.getFileDescriptor());
//    FileChannel fileChannel = inputStream.getChannel();
//    long startOffset = fileDescriptor.getStartOffset();
//    long declaredLength = fileDescriptor.getDeclaredLength();
//    return fileChannel.map(FileChannel.MapMode.READ_ONLY, startOffset, declaredLength);
  } // loadAssetMappedByteBuffer
